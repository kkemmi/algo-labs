...
class BinaryTree:
    ...
    def _build_canvas(self):
        ...
        for (pr, pc, cr, cc) in connections:
            px, chx = cx(pc), cx(cc)
            val_p, val_c = node_grid[pr][pc], node_grid[cr][cc]
            half_p, half_c = len(val_p) // 2 + 1, len(val_c) // 2 + 1
            p_canvas, c_canvas = canvas_row(pr), canvas_row(cr)

            if pr == cr:
                row = p_canvas
                x0, x1 = (px + half_p, chx - half_c) if px < chx else (chx + half_c, px - half_p)
                for x in range(x0, x1 + 1):
                    if 0 <= x < W: canvas[row][x] = "-"
            else:
                child_above, child_left = (cr < pr), (cc < pc)
                conn_row = (p_canvas + c_canvas) // 2
                if child_left:
                    diag, diag_x = ("\\" if child_above else "/"), chx + half_c
